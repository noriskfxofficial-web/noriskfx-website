(() => {
    ...
    leads: [
      {
        ...
        status: 'New',
        priority: 'Warm',
        note: 'Asked about beginner path and risk management.'
      },
      {
        id: cryptoId(),
        name: 'Broker Inquiry',
        ...
        status: 'Follow-up',
        priority: 'Hot',
        note: 'Needs co-branded campaign explanation.'
      }
    ],
    posts: [
      {
        id: cryptoId(),
        type: 'Market Update',
        title: 'Gold weekly awareness update',
        status: 'Draft',
        body: 'Educational technical summary with support/resistance and risk disclaimer.'
      }
    ],
    events: [
      {
        ...
        time: '15:30',
        currency: 'USD',
        title: 'Economic data watch',
        impact: 'High',
        previous: '—',
        forecast: '—',
        actual: '—',
        note: 'Prepare weekly calendar post from Tradays.'
      }
    ],
    partners: [
      {
        id: cryptoId(),
        name: 'FP Markets',
        tone: 'Corporate blue / platform-first',
        note: 'Use as style inspiration only. Keep No Risk FX primary.'
      },
      {
        id: cryptoId(),
        name: 'One Royal',
        tone: 'Premium black / purple / yellow',
        note: 'Cinematic educational style, no partner contact info unless requested.'
      },
      {
        id: cryptoId(),
        name: 'Tickmill',
        tone: 'Dark red / market objects',
        note: 'Bold headlines, risk disclaimer at bottom.'
      }
    ],
    checks: [
  ...
  function isStaticAuthed() {
    return sessionStorage.getItem(STATIC_AUTH) === 'true';
  }

  function clearStaticAuth() {
    sessionStorage.removeItem(STATIC_AUTH);
  }
  ...
  function persistSoon() {
    ...
    saveTimer = setTimeout(() => {
      persistState().catch((e) => alert('Save failed: ' + e.message));
    }, 150);
  }
  ...
  function updateModeCard() {
    ...
    if (apiMode === 'api') {
      ...
      sideCard.innerHTML = '<b>Admin locked</b><p>سجّل دخول بالباسورد الموجود في Netlify Environment Variables لتفعيل لوحة الإدارة.</p>';
  ...
  form?.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      if (error) {
        error.textContent = 'Wrong username/password or backend is unavailable.';
      }
    }
  });
  ...
  function renderLeads() {
    ...
    if (!state.leads.length) {
      el.innerHTML = '<div class="empty-state">No leads yet.</div>';
      return;
    }

    el.innerHTML =
      `<div class="table-row head"><b>Name</b><b>Service</b><b>Source</b><b>Status</b><b></b></div>` +
      state.leads.map((item) => `
        <div class="table-row">
          ...
          <div class="row-actions">
            <button class="icon-btn" data-edit="lead" data-id="${item.id}">✎</button>
            <button class="icon-btn" data-delete="lead" data-id="${item.id}">×</button>
          </div>
        </div>
      `).join('');
  }
    ...
    el.innerHTML = state.posts.length
      ? state.posts.map((item) => `
        <article class="content-item">
          ...
          <div class="row-actions">
            <button class="icon-btn" data-edit="post" data-id="${item.id}">✎</button>
            <button class="icon-btn" data-delete="post" data-id="${item.id}">×</button>
          </div>
        </article>
      `).join('')
      : '<div class="empty-state">No content drafts yet.</div>';
  }
  ...
  function renderEvents() {
    ...
    el.innerHTML =
      `<div class="table-row head calendar-head"><b>Date</b><b>Time</b><b>Currency</b><b>Event</b><b>Impact</b><b></b></div>` +
      state.events.map((item) => `
        <div class="table-row calendar-row">
          <strong>${escapeHtml(item.date)}</strong>
          <span>${escapeHtml(item.time || '—')}</span>
          <span>${escapeHtml(item.currency)}</span>
          <span>${escapeHtml(item.title)}</span>
          <span><i class="${badgeClass(item.impact)}">${escapeHtml(item.impact)}</i></span>
          <div class="row-actions">
            <button class="icon-btn" data-edit="event" data-id="${item.id}">✎</button>
  ...
  function renderPartners() {
    ...
    el.innerHTML = state.partners.length
      ? state.partners.map((item) => `
        <article class="partner-note glass">
          <strong>${escapeHtml(item.name)}</strong>
          <small>${escapeHtml(item.tone)}</small>
          <p>${escapeHtml(item.note)}</p>
          <div class="row-actions">
            <button class="icon-btn" data-edit="partner" data-id="${item.id}">✎</button>
            <button class="icon-btn" data-delete="partner" data-id="${item.id}">×</button>
          </div>
        </article>
      `).join('')
      : '<div class="empty-state">No partner notes yet.</div>';
  }
  ...
  function renderChecklist() {
    ...
    el.innerHTML = state.checks.map((item) => `
  ...
  function openDialog(type, itemId = null) {
    ...
    if (dialogTitle) {
      dialogTitle.textContent = (itemId ? 'Edit ' : 'Add ') + schema.label;
    }

    if (fields) {
      fields.innerHTML = `
        <div class="field-grid">
          ${schema.fields.map(([name, label, kind]) => {
            const value = escapeHtml(existing?.[name] || '');
            const input = kind === 'textarea'
              ? `<textarea name="${name}" rows="4">${value}</textarea>`
              : `<input type="${kind || 'text'}" name="${name}" value="${value}">`;
  ...
  document.addEventListener('click', (event) => {
    ...
    if (edit) {
      openDialog(edit.dataset.edit, edit.dataset.id);
    }
  ...
  dialogForm?.addEventListener('submit', (event) => {
    ...
    if (activeId) {
      state[schema.list] = state[schema.list].map((item) =>
        item.id === activeId ? { ...item, ...data } : item
      );
    } else {
  ...
  document.addEventListener('change', (event) => {
    ...
    if (check) {
      state.checks = state.checks.map((item) =>
        item.id === check.dataset.check ? { ...item, done: check.checked } : item
      );
      render();
    }
  });
  ...
  document.querySelector('[data-export]')?.addEventListener('click', async () => {
    try {
      ...
      if (getToken()) {
        payload = await api('/api/admin/export');
      } else {
        payload = {
          exportedAt: new Date().toISOString(),
          adminState: state,
          siteConfig: window.NoRiskFXCurrentSiteConfig || window.NoRiskFXSiteConfigDefaults
        };
      }
    ...
    try {
      ...
      if (getToken()) {
        await api('/api/admin/import', {
          method: 'POST',
          body: JSON.stringify(
            payload.adminState || payload.siteConfig ? payload : { adminState: payload }
          )
        });
...
(() => {
  ...
  const joinSymbols = (symbols = []) => symbols
    .map((item) => `${item.proName} | ${item.title || item.proName}`)
    .join('\n');
  ...
  async function loadConfig() {
    if (window.NoRiskFXLoadSiteConfig) {
      return mergeConfig(defaults, await window.NoRiskFXLoadSiteConfig());
    }

    try {
      return mergeConfig(defaults, JSON.parse(localStorage.getItem(KEY)) || {});
    } catch (e) {
  ...
  async function saveConfig(config) {
    ...
    if (adminApi?.getToken?.()) {
      await adminApi.api('/api/admin/site-config', {
        method: 'PUT',
  ...
  async function buildConfig() {
    ...
    return mergeConfig(base, {
      copy: {
        heroEyebrow: {
          ar: fd.get('heroEyebrowAr') || '',
          en: fd.get('heroEyebrowEn') || ''
        },
        heroHeadline: {
          ar: fd.get('heroHeadlineAr') || '',
          en: fd.get('heroHeadlineEn') || ''
        },
        heroLead: {
          ar: fd.get('heroLeadAr') || '',
          en: fd.get('heroLeadEn') || ''
        },
        weeklyTitle: {
          ar: fd.get('weeklyTitleAr') || '',
          en: base.copy?.weeklyTitle?.en || defaults.copy?.weeklyTitle?.en || ''
        },
        weeklyLead: {
          ar: fd.get('weeklyLeadAr') || '',
          en: base.copy?.weeklyLead?.en || defaults.copy?.weeklyLead?.en || ''
        },
        weeklyNote: {
          ar: fd.get('weeklyNoteAr') || '',
          en: base.copy?.weeklyNote?.en || defaults.copy?.weeklyNote?.en || ''
        }
      },
      stats: {
        support: {
          value: fd.get('statSupportValue') || '',
          label: {
            ar: fd.get('statSupportAr') || '',
            en: base.stats?.support?.label?.en || defaults.stats?.support?.label?.en || ''
          }
        },
        services: {
          value: fd.get('statServicesValue') || '',
          label: {
            ar: fd.get('statServicesAr') || '',
            en: base.stats?.services?.label?.en || defaults.stats?.services?.label?.en || ''
          }
        },
        partners: {
          value: fd.get('statPartnersValue') || '',
          label: {
            ar: fd.get('statPartnersAr') || '',
            en: base.stats?.partners?.label?.en || defaults.stats?.partners?.label?.en || ''
          }
        },
        trust: {
          value: fd.get('statTrustValue') || '',
          label: {
            ar: fd.get('statTrustAr') || '',
            en: base.stats?.trust?.label?.en || defaults.stats?.trust?.label?.en || ''
          }
        }
      },
      widgets: {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      alert(
        window.NoRiskFXAdminAPI?.getToken?.()
          ? 'Website changes saved to backend database.'
          : 'Website changes saved locally. Login with the Netlify backend to publish for all visitors.'
      );
    } catch (e) {
  ...
  document.querySelector('[data-site-export]')?.addEventListener('click', async () => {
    const blob = new Blob([JSON.stringify(await loadConfig(), null, 2)], {
      type: 'application/json'
    });
