...
const API_VERSION = 'password-fix-v4';
  ...
  events: [
    {
      ...
      forecast: '—',
      actual: '—',
      note: 'Add this week’s Tradays events here from the admin panel.'
    },
    {
      id: 'event-eur-1',
      date: new Date().toISOString().slice(0, 10),
      time: '12:00',
      currency: 'EUR',
      title: 'Eurozone calendar watch',
      impact: 'Medium',
      previous: '—',
      forecast: '—',
      actual: '—',
      note: 'Educational awareness only.'
    }
  ],
  partners: [
    {
...
global.__NORISKFX_MEMORY_DB = global.__NORISKFX_MEMORY_DB || null;
...
function defaultDb() {
  return {
    site_config: clone(siteDefaults),
    admin_state: clone(adminSeed),
    updated_at: now(),
    version: 17
  };
}
...
function sanitizeSiteConfig(input = {}) {
  ...
  cfg.widgets.tickerSymbols = Array.isArray(cfg.widgets.tickerSymbols)
    ? cfg.widgets.tickerSymbols
        ...
        .map((item) => ({
          proName: item.proName.trim(),
          title: String(item.title || item.proName).trim()
        }))
...
async function readDb(event) {
  ...
  if (!db) {
    db = global.__NORISKFX_MEMORY_DB || defaultDb();
  }
...
exports.handler = async (event) => {
  ...
  try {
    if (method === 'GET' && path === '/health') {
      return json(200, {
        ok: true,
        service: 'No Risk FX Netlify API',
        version: API_VERSION,
        storage: getStore ? 'netlify-blobs' : 'memory-fallback',
        adminUserFromEnv: Boolean(process.env.ADMIN_USER),
    ...
    if (method === 'POST' && path === '/auth/login') {
      ...
      if (!ADMIN_PASSWORD) {
        return json(500, {
          ...
          hint: 'Set ADMIN_PASSWORD in Netlify Environment Variables and redeploy.'
        });
      }
      ...
      if (!JWT_SECRET) {
        return json(500, {
          ...
          hint: 'Set JWT_SECRET in Netlify Environment Variables and redeploy.'
        });
      }

      if (username !== ADMIN_USER || !safeCompare(password, ADMIN_PASSWORD)) {
    ...
    if (method === 'GET' && path === '/public/weekly-events') {
      ...
      return json(200, {
        events: publicEvents(db.admin_state),
        source: db.site_config.widgets?.tradaysSourceUrl || siteDefaults.widgets.tradaysSourceUrl,
    ...
    if (method === 'GET' && path === '/admin/export') {
      ...
      return json(200, {
        exportedAt: now(),
        siteConfig: db.site_config,
        adminState: db.admin_state
      });
    }
    ...
    const status = /token|expired|invalid|unauthorized/i.test(error.message || '') ? 401 : 500;
